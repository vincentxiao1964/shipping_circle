...
import { getUserFollowersPage, type UserProfile } from "../../services/users";
import { syncPageI18n, t, type MessageKey } from "../../utils/i18n";

const I18N_KEYS = ["followers.title", "followers.empty", "common.failed", "common.noMore", "common.refresh", "user.follow", "user.unfollow"] as const satisfies readonly MessageKey[];
...
Page({
  ...
  onShow() {
    syncPageI18n(this, I18N_KEYS);
    wx.setNavigationBarTitle({ title: t("followers.title") });
    this.loadFirstPage();
  },
  onPullDownRefresh() {
  ...
  loadFirstPage() {
    ...
    this.setData({ loading: true });
    return getUserFollowersPage({ id: this.data.id, limit: PAGE_LIMIT })
      .then((page) => {
  ...
  loadMore() {
    ...
    this.setData({ loading: true });
    getUserFollowersPage({ id: this.data.id, limit: PAGE_LIMIT, cursor: this.data.cursor ?? undefined })
      .then((page) => {
        const me = this.data.meUserId;
        const rows: UserRow[] = (page?.items ?? []).map((u) => ({
          ...u,
